...
import { STAKINGFRONTEND_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof STAKINGFRONTEND_PROGRAM_ADDRESS,
  TAccountStakingfrontend extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountStakingfrontend extends string
        ? WritableAccount<TAccountStakingfrontend>
        : TAccountStakingfrontend,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountStakingfrontend extends string = string> = {
  stakingfrontend: Address<TAccountStakingfrontend>;
};

export function getDecrementInstruction<
  TAccountStakingfrontend extends string,
  TProgramAddress extends Address = typeof STAKINGFRONTEND_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountStakingfrontend>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountStakingfrontend> {
  // Program address.
  const programAddress =
    config?.programAddress ?? STAKINGFRONTEND_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    stakingfrontend: { value: input.stakingfrontend ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.stakingfrontend)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountStakingfrontend>;
...
> = {
  ...
  accounts: {
    stakingfrontend: TAccountMetas[0];
  };
...
>(
  ...
  return {
    ...
    accounts: {
      stakingfrontend: getNextAccount(),
    },
